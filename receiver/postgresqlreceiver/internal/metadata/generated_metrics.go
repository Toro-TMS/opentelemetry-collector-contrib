...
const (
	...
	AttributeSourceBlocksRead
	AttributeSourceBlocksHit
...
func (av AttributeSource) String() string {
	switch av {
	case AttributeSourceBlocksRead:
		return "blocks_read"
	case AttributeSourceBlocksHit:
		return "blocks_hit"
...
var MapAttributeSource = map[string]AttributeSource{
	"blocks_read": AttributeSourceBlocksRead,
	"blocks_hit":  AttributeSourceBlocksHit,
...
func (m *metricPostgresqlRowsFetched) init() {
	...
	m.data.SetDescription("The number of rows fetched by queries to the database.")
	m.data.SetUnit("1")
	m.data.SetEmptySum()
	m.data.Sum().SetIsMonotonic(true)
	m.data.Sum().SetAggregationTemporality(pmetric.AggregationTemporalityCumulative)
...
func (m *metricPostgresqlRowsReturned) init() {
	...
	m.data.SetDescription("The number of rows returned by queries to the database.")
	m.data.SetUnit("1")
	m.data.SetEmptySum()
	m.data.Sum().SetIsMonotonic(true)
	m.data.Sum().SetAggregationTemporality(pmetric.AggregationTemporalityCumulative)
